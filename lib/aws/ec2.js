...
function newEc2 (options) {
  ...
  return {
    ...
    listElasticIps: listElasticIps(ec2),
    listInstances: listInstances(ec2),
    runInstances: runInstances(ec2),
    startInstances: startInstances(ec2),
    stopInstances: stopInstances(ec2),
    terminateInstances: terminateInstances(ec2),
...
function startInstances (ec2) {
  return promised(ec2.startInstances.bind(ec2))
}

function stopInstances (ec2) {
  return promised(ec2.stopInstances.bind(ec2))
}

function terminateInstances (ec2) {
  return promised(ec2.terminateInstances.bind(ec2))
}
...
function listImages (ec2) {
  return promised(ec2.describeImages.bind(ec2))
}

function listElasticIps (ec2) {
  return promised(ec2.describeAddresses.bind(ec2))
}
